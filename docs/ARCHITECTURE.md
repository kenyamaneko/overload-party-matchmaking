...
キューからの取消は gateway プロセス内から送られる。gateway プロセスがそれを送る前に落ちると、待機不在のプレイヤーがキューに残り続け、後から来たプレイヤーがその不在の相手と成立してしまう。gateway は起動時に生成した識別子 (`gateway_instance_id`) を Enqueue のたびに送り、matchmaking は最後に受け取った値と比較する。異なれば別プロセスへの切り替わりとみなし、キュー全体を空にしてから登録する。

gateway は同時に 1 プロセスしか動かず、起動直後は接続を 1 つも持たない。したがって別プロセスからの最初の登録が来た時点で、キューに残っている全エントリは前のプロセスと一緒に死んだプレイヤーのものだと判定できる。比較・削除・識別子の更新・登録は Enqueue の Lua スクリプト内でアトミックに行う (詳細は [DATA_DESIGN.md](DATA_DESIGN.md))。
...
- pop 時点から書き戻しまでの間に gateway instance が切り替わっていた場合は書き戻さない。前のプロセスと一緒に接続が失われたプレイヤーであり、待機に戻しても救えないため、リトライは行わず Warn ログのみ記録して終える (「gatewayInstanceID によるキューリセット」)
