...
400 と 503 の応答本文は `{"error": <文言>}` を返す。400 は呼び出し側がリクエストを直せるよう、どの項目が原因かを区別できる文言を返す。503 は呼び出し側が対処できないため、matchmaking が所有する固定の文言だけを返し、原因はパス・メソッドとともに構造化ログへ記録する。いずれも解析ライブラリや Redis 由来の文字列は応答に載せない。
