...
**入力**: `playerID` (UUID v4 文字列), `deckID` (int64)
**出力**: 202 Accepted（ボディなし） / 400 / 503

### 2.1 仕様

1. `playerID` と `deckID` のバリデーション (空・0 は 400)
2. Redis Sorted Set `matchmaking:queue` に対し、**同一 playerID の既存メンバーを全削除してから** 新しいメンバー `<playerID>:<deckID>` をスコア `now().UnixMilli()` で ZADD
...
- Pub/Sub メッセージ属性 `matchId` に同じ値を入れる (subscriber 側の dedup key)

### 4.3 配信保証 (gateway との契約)

- **契約**: Exactly-Once (Cloud Pub/Sub Exactly-Once delivery を採用)
- **破れた場合の保険**: gateway の全 Pod が競合コンシューマとして subscribe し、`matchId` によるインメモリ重複排除を行う。battle 側のゲーム作成が同一 `matchId` に対して冪等であるため、万一 Exactly-Once が破れても二重ゲーム作成は発生しない
- publish 成功 = acknowledge。publish 前にキューから取り出した時点では acknowledge 扱いにしない (re-enqueue で巻き戻せる状態に保つ)

### 4.4 publish 失敗時のリカバリ契約
...
3. ドレイン中に publish 失敗が起きた場合、`ctx.Done()` 後も背景コンテキストで最終 re-enqueue を 1 回試行する (§4.4)
...
サービス層は HTTP ステータスを知らない。handler で以下に分類する:
