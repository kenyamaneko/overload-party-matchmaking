...
1. マッチングの走査で、`joinedAt` から 5 分以上経過したエントリを削除する
2. 5 分は gateway の待機タイムアウト (60 秒) に対する十分な余裕であり、正常な待機・取消はこの掃除に巻き込まれない
3. 削除件数を記録する
4. 掃除は先頭 2 件のペアリングより前に行い、期限切れのエントリが他プレイヤーとマッチすることを防ぐ
